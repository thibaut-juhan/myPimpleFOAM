...
int main(int argc, char *argv[])
{
	...
	else // steady state condition => find the time when is checked
	{
		...
		//runTime.setDeltaT(dt); // set a new value of dt
		while (pimple.correctNonOrthogonal()) 
		// resolve the advection-diffusion equation of passive scalar
		{
			fvScalarMatrix sEqn // Equation
			(
	 		fvm::ddt(s) + fvm::div(phi,s) -fvm::laplacian(DT,s) == fvOptions(s)
			//fvOptions => source term
			);		
			sEqn.solve(); // resolve the equation
		}
		// change bc : 
		label patchInlet                 = mesh.boundaryMesh().findPatchID("inlet"); 
		// inletPatch
		s.boundaryFieldRef()[patchInlet] == 1.; 
